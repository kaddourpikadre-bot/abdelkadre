...
checkoutBtn.addEventListener('click', ()=>{
  // PayPal payment via query form (client-only). تحتاج حساب PayPal: ضع بريدك التجاري في المتغير businessEmail
  const businessEmail = 'YOUR_PAYPAL_EMAIL@example.com'; // <-- غيّر هذا لبريد حساب البزنس على PayPal
  // إذا كانت السلة فارغة لا نفعل شيئاً
  const items = Object.values(cart);
  if(items.length === 0) return alert('السلة فارغة');

  // في هذا المثال سنحوّل الطلب لطلب واحد إجمالي (Command _xclick)؛ لمتابعة مفصّلة لكل عنصر يلزم ربط API أو زر لكل منتج
  ...
  // يمكنك إضافة return وnotify_url وshipping إلخ.
